...
const useReorderLinks = ({
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const response = await fetch("/api/v1/links/reorder", {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
    ...
    onError: (error: Error) => {
      const errMsg = t ? t(error.message) || error.message : error.message;
      toast?.error(errMsg || "Failed to reorder links.");
    },
  });
};
