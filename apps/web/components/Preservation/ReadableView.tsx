...
export default function ReadableView({ link }: Props) {
  ...
  useEffect(() => {
    setSelectionInfo((currentSelection) => {
      if (!currentSelection?.highlightId) return currentSelection;

      const comment =
        linkHighlights?.find((h) => h.id === currentSelection.highlightId)
          ?.comment || "";

      if (currentSelection.comment === comment) {
        return currentSelection;
      }

      return { ...currentSelection, comment };
    });
  }, [isCommenting, linkHighlights]);
  ...
  const highlightedHtml = React.useMemo(() => {
    if (!linkContent || !linkHighlights || linkHighlights.length === 0) {
      return linkContent;
    }
    ...
    const sortedHighlights = [...linkHighlights].sort(
