...
const CollectionListing = () => {
  ...
  const initialTree =
    collections.length > 0
      ? buildTreeFromCollections(collections, router, tree, user?.collectionOrder)
      : undefined;
...
const buildTreeFromCollections = (
  ...
  function getTotalLinkCount(collectionId: number, visited = new Set<number>()): number {
    ...
    if (collection.hasChildren) {
      collection.children.forEach((childId) => {
        totalLinkCount += getTotalLinkCount(childId as number, visited);
      });
    }
  ...
  Object.values(items).forEach((item) => {
    if (item.hasChildren && item.children.length > 1) {
      item.children.sort((a, b) => {
        const sortA = (items[a]?.data as any)?.sortOrder ?? 0;
        const sortB = (items[b]?.data as any)?.sortOrder ?? 0;
        return sortA - sortB;
      });
    }
  });
