...
export default function LinkDetails({
  ...
  const isOwnCollection = link.collection.ownerId === user?.id;

  const { data: remoteOwner } = useQuery<CollectionOwner>({
    queryKey: ["publicUser", link.collection.ownerId],
    queryFn: () => getPublicUserData(link.collection.ownerId as number),
    enabled: !isOwnCollection && !!link.collection.ownerId,
    staleTime: 1000 * 60 * 5,
  });

  const collectionOwner: CollectionOwner = isOwnCollection
    ? {
      ...
      >
        ...
        >
          {formatAvailable(link, "preview") ? (
            ...
            <div className="bg-base-200 duration-100 h-40"></div>
          ) : (
