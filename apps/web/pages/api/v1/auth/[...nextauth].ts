  ...
  providers.push(
    CredentialsProvider({
      ...
      async authorize(credentials, req) {
        ...
        if (!rateLimit(rateLimitKey, 5, 5 * 60 * 1000)) {
          auditLog({
            event: "auth.rate_limited",
            detail: `username=${username.toLowerCase()}`,
          });
          throw Error("Too many login attempts. Please try again later.");
        }
        ...
        // Use identical error message for user-not-found and wrong-password
